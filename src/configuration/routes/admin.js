...
module.exports = function (app) {
    ...
    app.get('/admin/userList', (req, res) => {
        ...
        common.database.executeQuery('SELECT user_id, last_name, first_name, active, role FROM users WHERE user_id != \'root\' ORDER BY last_name, first_name')
            .then(result =>
                res.send(result)
    ...
    app.post('/admin/changePassword', (req, res) => {
        ...
        if (req.user == null || req.user.role != 'A') {
            ...
            common.database.executeQuery('SELECT COUNT(*) CNT FROM users WHERE user_id = ?', req.body.params['user_id']).then(result => {
                ...
                var hash = bcryptjs.hashSync(req.body.params['password'], salt);

                common.database.executeQuery('UPDATE users SET password_hash = ?, updated_by = ?, updated_datetime = NOW() WHERE user_id = ?', [hash, req.user.username, req.body.params['user_id']]).then(
                    result => {

                        res.send(null);
                    }).catch(error => {
                        
                            common.res.sendErrorResponse(res, 400, error);
                        });
    ...
    app.delete('/admin/userData', (req, res) => {
        ...
        if (req.user == null || req.user.role != 'A') {
        ...
        else if (req.query['user_id'] == 'root') {
            common.sendErrorResponse(res, 401, 'The requested operation is not permitted.');
            return;
        }
        else {
            ...
            common.database.executeQuery('SELECT COUNT(*) CNT FROM users WHERE user_id = ?', req.query['user_id']).then(result => {
                ...
                common.database.executeQuery('DELETE FROM users WHERE user_id = ?', req.query['user_id']).then(
                    result => {
                        res.send(null);
